...
class KeysetPagerStrategy {
    ...
    fromCursorArgs(cursor) {
        ...
        let payload;
        let limit = 0;
        if (helpers_1.isForwardPaging(cursor)) {
    ...
    createQuery(query, opts, includeExtraNode) {
        ...
        const filter = core_1.mergeFilter((_a = query.filter) !== null && _a !== void 0 ? _a : {}, this.createFieldsFilter(sorting, payload));
    ...
    get defaultSort() {
        return this.pageFields.map((field) => {
            return { field, direction: core_1.SortDirection.ASC };
        });
    }
    encodeCursor(fields) {
    ...
    decodeCursor(cursor) {
        try {
            ...
            const partial = payload.fields.reduce((dtoPartial, { field, value }) => {
                return { ...dtoPartial, [field]: value };
            }, {});
            const transformed = class_transformer_1.plainToClass(this.DTOClass, partial);
    ...
    getSortFields(query, opts) {
        ...
        return opts.isForward ? sortFields : core_1.invertSort(sortFields);
    }
