...
describe('AssemblerDeserializer decorator', () => {
    it('should register a serializer', () => {
        ...
        TestSerializer = tslib_1.__decorate([
            src_1.AssemblerDeserializer((obj) => ({ foo: obj.bar }))
        ], TestSerializer);
        expect(assembler_deserializer_1.getAssemblerDeserializer(TestSerializer)({ bar: 'bar' })).toEqual({ foo: 'bar' });
    });
    it('should throw an error if the serializer is registered twice', () => {
        ...
        TestSerializer = tslib_1.__decorate([
            src_1.AssemblerDeserializer(deserializer)
        ], TestSerializer);
        expect(() => src_1.AssemblerDeserializer(deserializer)(TestSerializer)).toThrow('Assembler Deserializer already registered for TestSerializer');
    });
});
