...
class ReferenceQueryService {
    ...
    getReferenceModel(refName) {
        if (this.isReferencePath(refName)) {
            ...
            if (mongoose_types_helper_1.isEmbeddedSchemaTypeOptions(schemaType)) {
                return this.Model.model(schemaType.$embeddedSchemaType.options.ref);
            }
            if (mongoose_types_helper_1.isSchemaTypeWithReferenceOptions(schemaType)) {
                return this.Model.model(schemaType.options.ref);
            }
        }
        else if (this.isVirtualPath(refName)) {
            const schemaType = this.Model.schema.virtualpath(refName);
            if (mongoose_types_helper_1.isVirtualTypeWithReferenceOptions(schemaType)) {
                return this.Model.model(schemaType.options.ref);
            }
        }
    ...
    getReferenceFilter(refName, entity, filter) {
        ...
        if (this.isVirtualPath(refName)) {
            ...
            if (mongoose_types_helper_1.isVirtualTypeWithReferenceOptions(virtualType)) {
