...
describe('AggregateRelationsLoader', () => {
    describe('createLoader', () => {
        ...
        it('should return a function that accepts aggregate args', () => {
            const service = ts_mockito_1.mock();
            const queryRelationsLoader = new loader_1.AggregateRelationsLoader(RelationDTO, 'relation');
            expect(queryRelationsLoader.createLoader(ts_mockito_1.instance(service))).toBeInstanceOf(Function);
        });
        it('should try to load the relations with the query args', () => {
            const service = ts_mockito_1.mock();
            const aggregateRelationsLoader = new loader_1.AggregateRelationsLoader(RelationDTO, 'relation').createLoader(ts_mockito_1.instance(service));
            const filter = {};
            const aggregate = { count: ['id'] };
            const dtos = [{ id: 'dto-1' }, { id: 'dto-2' }];
            const dto1Aggregate = { count: { id: 2 } };
            const dto2Aggregate = { count: { id: 3 } };
            ts_mockito_1.when(service.aggregateRelations(RelationDTO, 'relation', ts_mockito_1.deepEqual(dtos), ts_mockito_1.deepEqual(filter), ts_mockito_1.deepEqual(aggregate))).thenResolve(new Map([
        ...
        it('should try return an empty aggregate result for each dto if no results are found', () => {
            ...
            const dtos = [{ id: 'dto-1' }, { id: 'dto-2' }];
            const dto1Aggregate = { count: { id: 2 } };
            ts_mockito_1.when(service.aggregateRelations(RelationDTO, 'relation', ts_mockito_1.deepEqual(dtos), ts_mockito_1.deepEqual(filter), ts_mockito_1.deepEqual(aggregate))).thenResolve(new Map([[dtos[0], dto1Aggregate]]));
            return expect(aggregateRelationsLoader([
                { dto: dtos[0], filter, aggregate },
                { dto: dtos[1], filter, aggregate },
        ...
        it('should group queryRelations calls by filter and return in the correct order', () => {
            const service = ts_mockito_1.mock();
            const queryRelationsLoader = new loader_1.AggregateRelationsLoader(RelationDTO, 'relation').createLoader(ts_mockito_1.instance(service));
            ...
            ts_mockito_1.when(service.aggregateRelations(RelationDTO, 'relation', ts_mockito_1.deepEqual([dtos[0], dtos[2]]), ts_mockito_1.deepEqual(filter1), ts_mockito_1.deepEqual(aggregate))).thenResolve(new Map([
                [dtos[0], dto1Aggregate],
                [dtos[2], dto3Aggregate],
            ]));
            ts_mockito_1.when(service.aggregateRelations(RelationDTO, 'relation', ts_mockito_1.deepEqual([dtos[1], dtos[3]]), ts_mockito_1.deepEqual(filter2), ts_mockito_1.deepEqual(aggregate))).thenResolve(new Map([
                [dtos[1], dto2Aggregate],
        ...
        it('should group queryRelations calls by aggregate and return in the correct order', () => {
            ...
            ts_mockito_1.when(service.aggregateRelations(RelationDTO, 'relation', ts_mockito_1.deepEqual([dtos[0], dtos[2]]), ts_mockito_1.deepEqual(filter), ts_mockito_1.deepEqual(aggregate1))).thenResolve(new Map([
                [dtos[0], dto1Aggregate],
                [dtos[2], dto3Aggregate],
            ]));
            ts_mockito_1.when(service.aggregateRelations(RelationDTO, 'relation', ts_mockito_1.deepEqual([dtos[1], dtos[3]]), ts_mockito_1.deepEqual(filter), ts_mockito_1.deepEqual(aggregate2))).thenResolve(new Map([
                [dtos[1], dto2Aggregate],
                [dtos[3], dto4Aggregate],
            ]));
            return expect(queryRelationsLoader([
