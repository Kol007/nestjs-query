...
describe('helpers', () => {
    describe('transformAndValidate', () => {
        ...
        it('should not transform or validate a class that is already an instance', async () => {
            ...
            const v = await helpers_1.transformAndValidate(TestClass, instance);
            expect(v).toBe(instance);
        });
        it('should transform and validate an object into the class', async () => {
            const v = await helpers_1.transformAndValidate(TestClass, { int: 1 });
            expect(v).toBeInstanceOf(TestClass);
            expect(v.int).toBe(1);
            return expect(helpers_1.transformAndValidate(TestClass, { int: -1 })).rejects.toBeInstanceOf(common_1.BadRequestException);
        });
    });
});
