...
describe('QueryRelationsLoader', () => {
    describe('createLoader', () => {
        ...
        it('should try to load the relations with the query args', () => {
            const service = ts_mockito_1.mock();
            const queryRelationsLoader = new loader_1.QueryRelationsLoader(RelationDTO, 'relation').createLoader(ts_mockito_1.instance(service));
            ...
            ts_mockito_1.when(service.queryRelations(RelationDTO, 'relation', ts_mockito_1.deepEqual(dtos), ts_mockito_1.deepEqual({}))).thenResolve(new Map([
        ...
        it('should try return an empty array for each dto is no results are found', () => {
            ...
            const dtos = [{ id: 'dto-1' }, { id: 'dto-2' }];
            const dto1Relations = [{ id: 'relation-1' }, { id: 'relation-2' }];
            ts_mockito_1.when(service.queryRelations(RelationDTO, 'relation', ts_mockito_1.deepEqual(dtos), ts_mockito_1.deepEqual({}))).thenResolve(new Map([[dtos[0], dto1Relations]]));
            return expect(queryRelationsLoader([
                { dto: dtos[0], query: {} },
                { dto: dtos[1], query: {} },
        ...
        it('should group queryRelations calls by query and return in the correct order', () => {
            ...
            ts_mockito_1.when(service.queryRelations(RelationDTO, 'relation', ts_mockito_1.deepEqual([dtos[0], dtos[2]]), ts_mockito_1.deepEqual({ paging: { limit: 10 } }))).thenResolve(new Map([
                [dtos[0], dto1Relations],
                [dtos[2], dto3Relations],
            ]));
            ts_mockito_1.when(service.queryRelations(RelationDTO, 'relation', ts_mockito_1.deepEqual([dtos[1], dtos[3]]), ts_mockito_1.deepEqual({}))).thenResolve(new Map([
                [dtos[1], dto2Relations],
