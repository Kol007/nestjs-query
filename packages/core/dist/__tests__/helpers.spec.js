...
describe('transformSort', () => {
    it('should return undefined if sorting is undefined', () => {
        expect(src_1.transformSort(undefined, fieldMap)).toBeUndefined();
    });
    it('should transform the fields to the correct names', () => {
        ...
        expect(src_1.transformSort(dtoSort, fieldMap)).toEqual(entitySort);
    });
    it('should throw an error if the field name is not found', () => {
        ...
        expect(() => src_1.transformSort(dtoSort, fieldMap)).toThrow("No corresponding field found for 'lasts' when transforming SortField");
    });
});
describe('transformFilter', () => {
    it('should return undefined if filter is undefined', () => {
        expect(src_1.transformFilter(undefined, fieldMap)).toBeUndefined();
    });
    it('should transform the fields to the correct names', () => {
        const dtoFilter = {
            first: { eq: 'foo' },
        ...
        expect(src_1.transformFilter(dtoFilter, fieldMap)).toEqual(entityFilter);
    });
    ...
    it('should throw an error if the field name is not found', () => {
        ...
        expect(() => src_1.transformFilter(dtoFilter, fieldMap)).toThrow("No corresponding field found for 'lasts' when transforming Filter");
    });
});
describe('transformQuery', () => {
    it('should transform a Query', () => {
        ...
        expect(src_1.transformQuery(dtoQuery, fieldMap)).toEqual(entityQuery);
    });
});
describe('applyFilter', () => {
    it('should handle eq comparisons', () => {
        ...
        expect(src_1.applyFilter({ first: 'foo', last: 'bar' }, filter)).toBe(true);
        expect(src_1.applyFilter({ first: 'bar', last: 'foo' }, filter)).toBe(false);
    });
    it('should handle neq comparisons', () => {
        const filter = {
            first: { neq: 'foo' },
        };
        expect(src_1.applyFilter({ first: 'bar', last: 'foo' }, filter)).toBe(true);
        expect(src_1.applyFilter({ first: 'foo', last: 'bar' }, filter)).toBe(false);
    });
    it('should handle gt comparisons', () => {
        const filter = {
            first: { gt: 'b' },
        };
        expect(src_1.applyFilter({ first: 'c', last: 'foo' }, filter)).toBe(true);
        expect(src_1.applyFilter({ first: 'b', last: 'foo' }, filter)).toBe(false);
        expect(src_1.applyFilter({ first: 'a', last: 'bar' }, filter)).toBe(false);
    });
    it('should handle gte comparisons', () => {
        ...
        expect(src_1.applyFilter({ first: 'c', last: 'foo' }, filter)).toBe(true);
        expect(src_1.applyFilter({ first: 'b', last: 'foo' }, filter)).toBe(true);
        expect(src_1.applyFilter({ first: 'a', last: 'bar' }, filter)).toBe(false);
    });
    it('should handle lt comparisons', () => {
        ...
        expect(src_1.applyFilter({ first: 'a', last: 'foo' }, filter)).toBe(true);
        expect(src_1.applyFilter({ first: 'b', last: 'bar' }, filter)).toBe(false);
        expect(src_1.applyFilter({ first: 'c', last: 'bar' }, filter)).toBe(false);
    });
    it('should handle lte comparisons', () => {
        ...
        expect(src_1.applyFilter({ first: 'a', last: 'foo' }, filter)).toBe(true);
        expect(src_1.applyFilter({ first: 'b', last: 'bar' }, filter)).toBe(true);
        expect(src_1.applyFilter({ first: 'c', last: 'bar' }, filter)).toBe(false);
    });
    it('should handle like comparisons', () => {
        ...
        expect(src_1.applyFilter({ first: 'Foo', last: 'foo' }, filter)).toBe(true);
        expect(src_1.applyFilter({ first: 'FOO', last: 'bar' }, filter)).toBe(false);
        expect(src_1.applyFilter({ first: 'Foo Bar', last: 'foo' }, filter)).toBe(false);
        expect(src_1.applyFilter({ first: 'o bar', last: 'bar' }, filter)).toBe(false);
    });
    it('should handle notLike comparisons', () => {
        const filter = {
            first: { notLike: '%oo' },
        };
        expect(src_1.applyFilter({ first: 'Foo', last: 'foo' }, filter)).toBe(false);
        expect(src_1.applyFilter({ first: 'FOO', last: 'bar' }, filter)).toBe(true);
        expect(src_1.applyFilter({ first: 'Foo Bar', last: 'foo' }, filter)).toBe(true);
        expect(src_1.applyFilter({ first: 'o bar', last: 'bar' }, filter)).toBe(true);
    });
    ...
    it('should handle in comparisons', () => {
        ...
        expect(src_1.applyFilter({ first: 'Foo', last: 'foo' }, filter)).toBe(true);
        expect(src_1.applyFilter({ first: 'Bar', last: 'bar' }, filter)).toBe(true);
        expect(src_1.applyFilter({ first: 'Baz', last: 'foo' }, filter)).toBe(true);
        expect(src_1.applyFilter({ first: 'Boo', last: 'bar' }, filter)).toBe(false);
    });
    it('should handle notIn comparisons', () => {
        ...
        expect(src_1.applyFilter({ first: 'Foo', last: 'foo' }, filter)).toBe(false);
        expect(src_1.applyFilter({ first: 'Bar', last: 'bar' }, filter)).toBe(false);
        expect(src_1.applyFilter({ first: 'Baz', last: 'foo' }, filter)).toBe(false);
        expect(src_1.applyFilter({ first: 'Boo', last: 'bar' }, filter)).toBe(true);
    });
    it('should handle between comparisons', () => {
        const filter = {
            first: { between: { lower: 'b', upper: 'd' } },
        };
        expect(src_1.applyFilter({ first: 'a', last: 'foo' }, filter)).toBe(false);
        expect(src_1.applyFilter({ first: 'b', last: 'bar' }, filter)).toBe(true);
        expect(src_1.applyFilter({ first: 'c', last: 'foo' }, filter)).toBe(true);
        expect(src_1.applyFilter({ first: 'd', last: 'bar' }, filter)).toBe(true);
        expect(src_1.applyFilter({ first: 'e', last: 'bar' }, filter)).toBe(false);
    });
    it('should handle notBetween comparisons', () => {
        ...
        expect(src_1.applyFilter({ first: 'a', last: 'foo' }, filter)).toBe(true);
        expect(src_1.applyFilter({ first: 'b', last: 'bar' }, filter)).toBe(false);
        expect(src_1.applyFilter({ first: 'c', last: 'foo' }, filter)).toBe(false);
        expect(src_1.applyFilter({ first: 'd', last: 'bar' }, filter)).toBe(false);
        expect(src_1.applyFilter({ first: 'e', last: 'bar' }, filter)).toBe(true);
    });
    it('should throw an error for an unknown operator', () => {
        ...
        expect(() => src_1.applyFilter({ first: 'baz', last: 'kaz' }, filter)).toThrow('unknown comparison "foo"');
    });
    it('should handle and grouping', () => {
        ...
        expect(src_1.applyFilter({ first: 'foo', last: 'bar' }, filter)).toBe(true);
        expect(src_1.applyFilter({ first: 'foo', last: 'foobar' }, filter)).toBe(true);
        expect(src_1.applyFilter({ first: 'oo', last: 'bar' }, filter)).toBe(false);
        expect(src_1.applyFilter({ first: 'foo', last: 'baz' }, filter)).toBe(false);
    });
    it('should handle or grouping', () => {
        ...
        expect(src_1.applyFilter({ first: 'foo', last: 'bar' }, filter)).toBe(true);
        expect(src_1.applyFilter({ first: 'foo', last: 'foobar' }, filter)).toBe(true);
        expect(src_1.applyFilter({ first: 'oo', last: 'bar' }, filter)).toBe(true);
        expect(src_1.applyFilter({ first: 'foo', last: 'baz' }, filter)).toBe(true);
        expect(src_1.applyFilter({ first: 'fo', last: 'ba' }, filter)).toBe(false);
    });
    describe('nested objects', () => {
        ...
        it('should handle like comparisons', () => {
            ...
            expect(src_1.applyFilter(withChild({ first: 'afoo' }), parentFilter)).toBe(true);
            expect(src_1.applyFilter(withChild({ first: 'bar' }), parentFilter)).toBe(false);
            expect(src_1.applyFilter(withGrandChild({ first: 'afoo' }), grandParentFilter)).toBe(true);
            expect(src_1.applyFilter(withGrandChild({ first: 'bar' }), grandParentFilter)).toBe(false);
        });
        it('should handle notLike comparisons', () => {
            const parentFilter = { child: { first: { notLike: '%foo' } } };
            const grandParentFilter = { child: { child: { first: { notLike: '%foo' } } } };
            expect(src_1.applyFilter(withChild({ first: 'bar' }), parentFilter)).toBe(true);
            expect(src_1.applyFilter(withChild({ first: 'afoo' }), parentFilter)).toBe(false);
            expect(src_1.applyFilter(withGrandChild({ first: 'bar' }), grandParentFilter)).toBe(true);
            expect(src_1.applyFilter(withGrandChild({ first: 'afoo' }), grandParentFilter)).toBe(false);
        });
        it('should handle iLike comparisons', () => {
        ...
        it('should handle in comparisons', () => {
            ...
            expect(src_1.applyFilter(withChild({ first: 'foo' }), parentFilter)).toBe(true);
            expect(src_1.applyFilter(withChild({ first: 'bar' }), parentFilter)).toBe(false);
            expect(src_1.applyFilter(withGrandChild({ first: 'foo' }), grandParentFilter)).toBe(true);
            expect(src_1.applyFilter(withGrandChild({ first: 'bar' }), grandParentFilter)).toBe(false);
        });
        it('should handle notIn comparisons', () => {
            ...
            expect(src_1.applyFilter(withChild({ first: 'bar' }), parentFilter)).toBe(true);
            expect(src_1.applyFilter(withChild({ first: 'foo' }), parentFilter)).toBe(false);
            expect(src_1.applyFilter(withGrandChild({ first: 'bar' }), grandParentFilter)).toBe(true);
            expect(src_1.applyFilter(withGrandChild({ first: 'foo' }), grandParentFilter)).toBe(false);
        });
        it('should handle between comparisons', () => {
            ...
            expect(src_1.applyFilter(withChild({ first: 'b' }), parentFilter)).toBe(true);
            expect(src_1.applyFilter(withChild({ first: 'd' }), parentFilter)).toBe(false);
            expect(src_1.applyFilter(withGrandChild({ first: 'b' }), grandParentFilter)).toBe(true);
            expect(src_1.applyFilter(withGrandChild({ first: 'd' }), grandParentFilter)).toBe(false);
        });
        it('should handle notBetween comparisons', () => {
            ...
            expect(src_1.applyFilter(withChild({ first: 'd' }), parentFilter)).toBe(true);
            expect(src_1.applyFilter(withChild({ first: 'b' }), parentFilter)).toBe(false);
            expect(src_1.applyFilter(withGrandChild({ first: 'd' }), grandParentFilter)).toBe(true);
            expect(src_1.applyFilter(withGrandChild({ first: 'b' }), grandParentFilter)).toBe(false);
        });
        it('should handle gt comparisons', () => {
            ...
            expect(src_1.applyFilter(withChild({ first: 'd' }), parentFilter)).toBe(true);
            expect(src_1.applyFilter(withChild({ first: 'b' }), parentFilter)).toBe(false);
            expect(src_1.applyFilter(withChild({ first: 'c' }), parentFilter)).toBe(false);
            expect(src_1.applyFilter(withGrandChild({ first: 'd' }), grandParentFilter)).toBe(true);
            expect(src_1.applyFilter(withGrandChild({ first: 'b' }), grandParentFilter)).toBe(false);
            expect(src_1.applyFilter(withGrandChild({ first: 'c' }), grandParentFilter)).toBe(false);
        });
        it('should handle gte comparisons', () => {
            const parentFilter = { child: { first: { gte: 'c' } } };
            const grandParentFilter = { child: { child: { first: { gte: 'c' } } } };
            expect(src_1.applyFilter(withChild({ first: 'c' }), parentFilter)).toBe(true);
            expect(src_1.applyFilter(withChild({ first: 'd' }), parentFilter)).toBe(true);
            expect(src_1.applyFilter(withChild({ first: 'b' }), parentFilter)).toBe(false);
            expect(src_1.applyFilter(withGrandChild({ first: 'c' }), grandParentFilter)).toBe(true);
            expect(src_1.applyFilter(withGrandChild({ first: 'd' }), grandParentFilter)).toBe(true);
            expect(src_1.applyFilter(withGrandChild({ first: 'b' }), grandParentFilter)).toBe(false);
        });
        it('should handle lt comparisons', () => {
        ...
        it('should handle is comparisons', () => {
            ...
            expect(src_1.applyFilter(withChild({ first: null }), parentFilter)).toBe(true);
            expect(src_1.applyFilter(withChild({}), parentFilter)).toBe(true); // undefined
            expect(src_1.applyFilter(withChild({ first: 'foo' }), parentFilter)).toBe(false);
            expect(src_1.applyFilter(withGrandChild({ first: null }), grandParentFilter)).toBe(true);
            expect(src_1.applyFilter(withGrandChild({}), grandParentFilter)).toBe(true); // undefined
            expect(src_1.applyFilter(withGrandChild({ first: 'foo' }), grandParentFilter)).toBe(false);
        });
        it('should handle isNot comparisons', () => {
            ...
            expect(src_1.applyFilter(withChild({ first: 'foo' }), parentFilter)).toBe(true);
            expect(src_1.applyFilter(withChild({ first: null }), parentFilter)).toBe(false);
            expect(src_1.applyFilter(withChild({}), parentFilter)).toBe(false); // undefined
            expect(src_1.applyFilter(withGrandChild({ first: 'foo' }), grandParentFilter)).toBe(true);
            expect(src_1.applyFilter(withGrandChild({ first: null }), grandParentFilter)).toBe(false);
            expect(src_1.applyFilter(withGrandChild({}), grandParentFilter)).toBe(false); // undefined
        });
    });
    describe('nested nulls', () => {
        ...
        it('should handle like comparisons', () => {
            expect(src_1.applyFilter(singleNestedNull(), { child: { first: { like: '%foo' } } })).toBe(false);
            expect(src_1.applyFilter(doubleNestedNull(), { child: { child: { first: { like: '%foo' } } } })).toBe(false);
        });
        it('should handle notLike comparisons', () => {
            expect(src_1.applyFilter(singleNestedNull(), { child: { first: { notLike: '%foo' } } })).toBe(true);
            expect(src_1.applyFilter(doubleNestedNull(), { child: { child: { first: { notLike: '%foo' } } } })).toBe(true);
        });
        it('should handle iLike comparisons', () => {
            expect(src_1.applyFilter(singleNestedNull(), { child: { first: { iLike: '%foo' } } })).toBe(false);
            expect(src_1.applyFilter(doubleNestedNull(), { child: { child: { first: { iLike: '%foo' } } } })).toBe(false);
        });
        it('should handle notILike comparisons', () => {
            expect(src_1.applyFilter(singleNestedNull(), { child: { first: { notILike: '%foo' } } })).toBe(true);
            expect(src_1.applyFilter(doubleNestedNull(), { child: { child: { first: { notILike: '%foo' } } } })).toBe(true);
        });
        it('should handle in comparisons', () => {
            expect(src_1.applyFilter(singleNestedNull(), { child: { first: { in: ['foo'] } } })).toBe(false);
            expect(src_1.applyFilter(doubleNestedNull(), { child: { child: { first: { in: ['foo'] } } } })).toBe(false);
        });
        it('should handle notIn comparisons', () => {
            expect(src_1.applyFilter(singleNestedNull(), { child: { first: { notIn: ['foo'] } } })).toBe(true);
            expect(src_1.applyFilter(doubleNestedNull(), { child: { child: { first: { notIn: ['foo'] } } } })).toBe(true);
        });
        it('should handle between comparisons', () => {
            expect(src_1.applyFilter(singleNestedNull(), { child: { first: { between: { lower: 'foo', upper: 'bar' } } } })).toBe(false);
            expect(src_1.applyFilter(doubleNestedNull(), { child: { child: { first: { between: { lower: 'foo', upper: 'bar' } } } } })).toBe(false);
        });
        it('should handle notBetween comparisons', () => {
            expect(src_1.applyFilter(singleNestedNull(), { child: { first: { notBetween: { lower: 'foo', upper: 'bar' } } } })).toBe(true);
            expect(src_1.applyFilter(doubleNestedNull(), {
                child: { child: { first: { notBetween: { lower: 'foo', upper: 'bar' } } } },
            })).toBe(true);
        });
        it('should handle gt comparisons', () => {
            expect(src_1.applyFilter(singleNestedNull(), { child: { first: { gt: 'foo' } } })).toBe(false);
            expect(src_1.applyFilter(doubleNestedNull(), { child: { child: { first: { gt: 'foo' } } } })).toBe(false);
        });
        it('should handle gte comparisons', () => {
            expect(src_1.applyFilter(singleNestedNull(), { child: { first: { gte: 'foo' } } })).toBe(false);
            expect(src_1.applyFilter(doubleNestedNull(), { child: { child: { first: { gte: 'foo' } } } })).toBe(false);
        });
        it('should handle lt comparisons', () => {
            expect(src_1.applyFilter(singleNestedNull(), { child: { first: { lt: 'foo' } } })).toBe(false);
            expect(src_1.applyFilter(doubleNestedNull(), { child: { child: { first: { lt: 'foo' } } } })).toBe(false);
        });
        it('should handle lte comparisons', () => {
            expect(src_1.applyFilter(singleNestedNull(), { child: { first: { lte: 'foo' } } })).toBe(false);
            expect(src_1.applyFilter(doubleNestedNull(), { child: { child: { first: { lte: 'foo' } } } })).toBe(false);
        });
        it('should handle eq comparisons', () => {
            expect(src_1.applyFilter(singleNestedNull(), { child: { first: { eq: 'foo' } } })).toBe(false);
            expect(src_1.applyFilter(doubleNestedNull(), { child: { child: { first: { eq: 'foo' } } } })).toBe(false);
        });
        it('should handle neq comparisons', () => {
            expect(src_1.applyFilter(singleNestedNull(), { child: { first: { neq: 'foo' } } })).toBe(true);
            expect(src_1.applyFilter(doubleNestedNull(), { child: { child: { first: { neq: 'foo' } } } })).toBe(true);
        });
        it('should handle is comparisons', () => {
            expect(src_1.applyFilter(singleNestedNull(), { child: { first: { is: null } } })).toBe(true);
            expect(src_1.applyFilter(doubleNestedNull(), { child: { child: { first: { is: null } } } })).toBe(true);
        });
        it('should handle isNot comparisons', () => {
            expect(src_1.applyFilter(singleNestedNull(), { child: { first: { isNot: null } } })).toBe(false);
            expect(src_1.applyFilter(doubleNestedNull(), { child: { child: { first: { isNot: null } } } })).toBe(false);
        });
    });
});
describe('getFilterFields', () => {
    ...
    it('should get all fields at root of filter', () => {
        ...
        expect(src_1.getFilterFields(filter).sort()).toEqual(['boolField', 'strField', 'testRelation']);
    });
...
describe('transformAggregateQuery', () => {
    it('should transform an aggregate query', () => {
        ...
        expect(src_1.transformAggregateQuery(aggQuery, fieldMap)).toEqual(entityAggQuery);
    });
    it('should throw an error if an unknown field is encountered', () => {
        ...
        // @ts-ignore
        expect(() => src_1.transformAggregateQuery(aggQuery, { last: 'lastName' })).toThrow("No corresponding field found for 'first' when transforming aggregateQuery");
    });
});
describe('transformAggregateResponse', () => {
    it('should transform an aggregate query', () => {
        ...
        expect(src_1.transformAggregateResponse(aggResponse, fieldMap)).toEqual(entityAggResponse);
    });
    ...
    it('should throw an error if the field is not found', () => {
        ...
        // @ts-ignore
        expect(() => src_1.transformAggregateResponse(aggResponse, { last: 'lastName' })).toThrow("No corresponding field found for 'first' when transforming aggregateQuery");
        aggResponse = {
            max: {
                age: 10,
            },
        ...
        // @ts-ignore
        expect(() => src_1.transformAggregateResponse(aggResponse, { last: 'lastName' })).toThrow("No corresponding field found for 'age' when transforming aggregateQuery");
    });
});
describe('applySort', () => {
    ...
    describe('sort asc', () => {
        ...
        testCases.forEach(({ description, input, expected, sortFields }) => {
            it(`should ${description}`, () => {
                expect(src_1.applySort(input, sortFields)).toEqual(expected);
            });
        });
    });
...
describe('applyPaging', () => {
    ...
    testCases.forEach(({ description, input, expected, paging }) => {
        it(`should ${description}`, () => {
            expect(src_1.applyPaging(input, paging)).toEqual(expected);
        });
    });
});
describe('applyQuery', () => {
    ...
    testCases.forEach(({ description, input, expected, query }) => {
        it(`should ${description}`, () => {
            expect(src_1.applyQuery(input, query)).toEqual(expected);
        });
    });
});
describe('getFilterComparisons', () => {
    it('should get list of comparisons from a filter given a key', () => {
        ...
        expect(src_1.getFilterComparisons(f0, 'bar')).toEqual(expect.arrayContaining([]));
        expect(src_1.getFilterComparisons(f1, 'bar')).toEqual(expect.arrayContaining([{ gt: 0 }]));
        expect(src_1.getFilterComparisons(f2, 'bar')).toEqual(expect.arrayContaining([{ gt: 0 }, { lt: 3 }]));
        expect(src_1.getFilterComparisons(f3, 'bar')).toEqual(expect.arrayContaining([{ gt: 0 }, { lt: 5 }]));
        expect(src_1.getFilterComparisons(f4, 'bar')).toEqual(expect.arrayContaining([{ gt: 0 }, { lt: 3 }, { lt: 5 }]));
    });
});
describe('getFilterOmitting', () => {
    it('should omit a key from a filter', () => {
        ...
        expect(src_1.getFilterOmitting(filter, 'baz')).toEqual({
            bar: { gt: 0 },
            and: [{ bar: { lt: 100 } }],
            or: [{ bar: { lt: 100 } }],
        });
...
describe('mergeFilter', () => {
    it('should merge two filters', () => {
        ...
        expect(src_1.mergeFilter(f1, f2)).toEqual({
    ...
    it('should noop if one of the filters is empty', () => {
        ...
        expect(src_1.mergeFilter(filter, {})).toEqual(filter);
        expect(src_1.mergeFilter({}, filter)).toEqual(filter);
    });
});
